...
def test_basic_inference0():
    x = np.random.random([8,224,224,3]).astype(np.float32)

    module = elegy.nets.resnet.ResNet50(weights='imagenet')
    model  = elegy.Model(module)
    model.maybe_initialize('pred', x)
    state0      = model.states
    

    predict_jit = lambda x, state: model.call_pred_step_jit(x, 'pred', state, False)
    jaxpr       = jax.make_jaxpr(predict_jit)(x, state0)
    #print(jaxpr)

    predict_vk = vkjax.Function(predict_jit)

    predstep = predict_vk(x, state0)
    y        = predstep.y_pred
    ytrue    = model.predict(x)

    assert np.shape(y) == np.shape(ytrue)
    #XXX: need higher than default tolerances
    assert np.allclose(y, ytrue, atol=1e-5, rtol=1e-4)
